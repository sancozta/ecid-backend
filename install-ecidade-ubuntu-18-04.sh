...
sleep 2
apt-get update
apt-get --force-yes --yes install postgresql-9.5 postgresql-client-9.5 postgresql-contrib-9.5
...
sed -i 's/md5$/trust/g' /etc/postgresql/9.5/ecidade/pg_hba.conf
sed -i 's/peer$/trust/g' /etc/postgresql/9.5/ecidade/pg_hba.conf

PGPATH=/etc/postgresql/9.5/ecidade/postgresql.conf
